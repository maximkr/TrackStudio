...
public abstract class FJTask implements Runnable {
  ...
  public static void coInvoke(FJTask[] tasks) {
    ...
    public static void coInvoke(Collection tasks) {
        FJTask taskArr[] = new FJTask[tasks.size()];
        int i=0;

        for (Iterator it = tasks.iterator();it.hasNext();i++) {
            taskArr[i] = (FJTask)it.next();
        }
        
        getFJTaskRunner().coInvoke(taskArr);
    }
