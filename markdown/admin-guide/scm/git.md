...
[Git](http://ru.wikipedia.org/wiki/Git) — распределённая система управления версиями файлов. Проект был создан Линусом Торвальдсом для управления разработкой ядра Linux. Поддержка SCM Git реализована в TrackStudio, начиная с версии 4.0.11.

## Настройка параметров соединения

Вы можете настроить параметры соединения с вашим хранилищем Git в утилите TrackStudio Server Manager, либо, если ее нет — в любом текстовом редакторе.
...
- Выберите тип SCM: Git
- Укажите корень исходного кода проекта в Git
...
Если у вас отсутствует возможность запустить Server Manager, вы можете настроить интеграцию с Git в файле **trackstudio.scm.properties**
...
```
trackstudio.scm.MyGit.tasknum=1
```
...
```
trackstudio.scm.MyGit.type=Git
```

- Укажите корень исходного кода проекта в Git:

```
trackstudio.scm.MyGit.root=/home/winzard/git/
```

## Работа с TrackStudio

Чтобы ваши коммиты в Git связывались с конкретными задачами TrackStudio, добавляйте в сообщения к коммитам номер (или номера) соответствующих задач в системе.

Например:

```
$ git commit -am "#12 fixed some bugs"
```

После того, как TrackStudio обработает информацию, при просмотре задачи будут выводиться логи коммитов в Git
