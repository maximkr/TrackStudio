...
# Upgrading the minor versions (5.0.x)
...
4. Copy the entire content of directories etc, where the files like **property**, scripts, templates etc are stored (except **t****rackstudio.adapters.properties, trackstudio.hibernate.default.properties** and e-mail templates) into the analogous directory in the new version.
...
- If you are using default DBMS (HSQLDB), then the files with database are named as *tsdb.lck, tsdb.log, tsdb.properties** *and *tsdb.script.** *Do not delete them.
