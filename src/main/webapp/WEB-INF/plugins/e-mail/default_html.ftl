...
<div class="viewtask">
    <table class="general" cellpadding="4" cellspacing="1">
        ...
        <#if msg.description?exists>
            <tr>
                <td colspan="2">
                    <div class="description">${Wiki.toMacros(msg.description)}</div>
                </td>
            </tr>
        </#if>
    </table>
        ...
        <#if uValue?exists>
            ...
            <tr>
                ...
                <td><#if udf.value?exists>
                    ...
                    ${taskUDF.name} [#${taskUDF.number}]<br>
                    </#list>
                </#if>
                </td>
            </tr>
...
<#macro valueUdf udf>
    <#if udf?exists>
        ...
        <#if uValue?exists>
            <#if udf.type == 'date'>
                <#if uValue!="">${DateFormatter.parse(uValue)}</#if>
            </#if>
            <#if udf.type == 'float'>
            ${uValue}
            </#if>
            ...
            <#if udf.type == 'task'>
                <#if udf.value?exists>
                    <#list uValue as taskUDF>
                    ${taskUDF.name} [<em class="number"#${taskUDF.number}</em>]
                    </#list>
                </#if>
            </#if>
            <#if udf.type == 'user'>
...
<table class="change">
    ...
    ${Wiki.toMacros(task.description)}
    </div>
    </#if>
</#if>
<#if reason.code == "S">
    ...
    <tr>
        ...
        <td>
            ...
            <#if ((reason.message.getHandlerUserId()?exists || reason.message.getHandlerGroupId()?exists))>
                <#assign addMsgFor>
                    <#if reason.message.handlerGroup?exists>${reason.message.handlerGroup.name}<#else>${reason.message.handlerUser.name}</#if>
                </#assign>
...
<div class="newmessage">
    <#if reason.message?exists>
        <#if reason.message.description?exists>
        ${Wiki.toMacros(reason.message.description)}
        </#if>
                ...
                <div class="subtasks">
                    <table class="general" cellpadding="4" cellspacing="1">
                        <tr>
                            ...
                            <#list sortedUdfHeaderCaptionIds as udfId>
                                <th class="subs">${udfHeaderCaption[udfId]}</th>
                            </#list>
                        </tr>
...
${DateFormatter.parse(item.deadline)}
                                ...
                                <#list sortedUdfHeaderCaptionIds as udfId>
                                    <td class="subs">
                                        <#list item.getUdfValuesList() as udf>
                    <#if udf.id == udfId>
                                            <@valueUdf udf=udf/>
                                        </#if>
                </#list>
                            ...
                            <#if filter.MESSAGEVIEW>
                                <tr>
                                    <td class="subs" colspan="${totalCols+1}">
                                        <@std.I18n key="HISTORY"/>
                            <#list Util.getMessageByFilter(item, filterId) as msg>
                                        <@message msg=msg/>
                                    </#list>
                                    </td>
                                </tr>
...
<#assign taskMessages = Util.getMessageByFilter(task, filterId)/>
