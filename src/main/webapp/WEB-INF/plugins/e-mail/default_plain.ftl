...
<#assign view=ViewFactory.inEmailText(task)/>
...
>::${Wiki.toMacros(view,reason.message.textDescription)}
...
${Wiki.toMacros(view,task.textDescription)}
====================================================================================================
</#if>
<@table columns=2 positions="0,40,160">
<@std.I18n key="FULL_PATH"/>::    <#if task.parent?exists><@std.path to=task.parent.number ; number, name>/ ${name}</@std.path></#if>
    ...
    <#if task.handlerUserId?exists>
    <@std.I18n key="HANDLER"/>::      ${task.handler.name}
    </#if>
    <#if task.handlerGroupId?exists>
    <@std.I18n key="HANDLER"/>::      ${task.handlerGroup.name}
    </#if>
        ...
        <#list viewUdfList as udf>
            <#if udf?exists>
                ...
                <#if uValue?exists>
                    ...
                    <#if udf.type == 'string'>
                    ${udf.caption} ::                         ${Wiki.cutHtml(uValue)}
                    </#if>
                    <#if udf.type == 'integer'>
                    ${udf.caption} ::                         ${uValue}
                    </#if>
...
===============================================================================
    ...
    <@table columns=2 positions="0,30,80">
        ...
        ${udfHeaderCaption[udfId]}  : ${udfs[udfId]}
        </#list>
    </@table>
        <#if item.description?exists>
        ----------------------------------------------------------------------------------------------------
        <@std.I18n key="DESCRIPTION"/>:
        ${Wiki.toMacros(view,item.textDescription)}
        </#if>
    ====================================================================================================
    </#list>
</#if>
...
<#if (taskMessages?size>0)>
    ...
    ----------------------------------------------------------------------------------------------------
        ...
        ${Wiki.toMacros(view, msg.textDescription)}
        </@table>
        ====================================================================================================
        </#list>
    </#if>
