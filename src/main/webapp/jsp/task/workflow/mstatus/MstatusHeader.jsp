<%@ page buffer="128kb" errorPage="/jsp/Error.jsp"%>
<%@ taglib uri="jstl/c" prefix="c" %>
<%@ taglib uri="jstl/fmt" prefix="I18n" %>
<%@ taglib uri="struts/html-el" prefix="html" %>
<I18n:setLocale value='${sc.locale}'/>
<I18n:setTimeZone value='${sc.timezone}'/>
<I18n:setBundle basename='language'/>
<c:set var="mstatusUrl" value="${contextPath}/MstatusAction.do?method=page&id=${id}&workflowId=${flow.id}"/>
<c:if test="${!tabEdit.allowed}">
    <c:set var="mstatusUrl" value="${contextPath}/WorkflowViewAction.do?method=page&workflowId=${flow.id}&id=${id}"/>
</c:if>
<div class="ts-admin-header ts-admin-header--mstatus" id="customHeader">
    <div class="ts-admin-header__inner">
        <html:link styleClass="ts-admin-header__crumb" href="${contextPath}/WorkflowAction.do?method=page&id=${id}">
            <html:img src="${contextPath}${ImageServlet}/cssimages/ico.workflow.gif" hspace="0" vspace="0" border="0" />
            <I18n:message key="WORKFLOWS"/>
        </html:link>
        <span class="ts-admin-header__separator">/</span>
        <html:link styleClass="ts-admin-header__current" href="${contextPath}/WorkflowViewAction.do?method=page&workflowId=${flow.id}&id=${id}">
            <c:out value="${flow.name}" escapeXml="true"/>
        </html:link>
        <span class="ts-admin-header__separator">/</span>
        <html:link styleClass="ts-admin-header__crumb" href="${mstatusUrl}">
            <html:img src="${contextPath}${ImageServlet}/cssimages/ico.messagetypes.gif" hspace="0" vspace="0" border="0" />
            <I18n:message key="MESSAGE_TYPES"/>
        </html:link>
        <span class="ts-admin-header__separator">/</span>
        <c:choose>
            <c:when test="${mstatus ne null}">
                <html:link styleClass="ts-admin-header__current" href="${contextPath}/MstatusViewAction.do?method=page&mstatusId=${mstatus.id}&id=${id}&workflowId=${flow.id}">
                    <c:out value="${mstatus.name}" escapeXml="true"/>
                </html:link>
            </c:when>
            <c:otherwise>
                <span class="ts-admin-header__current ts-admin-header__current--new"><I18n:message key="MESSAGE_TYPE_ADD"/></span>
            </c:otherwise>
        </c:choose>
        <c:if test="${udf ne null}">
            <span class="ts-admin-header__separator">/</span>
            <html:link styleClass="ts-admin-header__crumb" href="${contextPath}/UdfPermissionAction.do?method=page&mstatusId=${mstatus.id}&id=${id}&workflowId=${flow.id}">
                <html:img src="${contextPath}${ImageServlet}/cssimages/ico.customfields.gif" hspace="0" vspace="0" border="0" />
                <I18n:message key="CUSTOM_FIELDS"/>
            </html:link>
            <span class="ts-admin-header__separator">/</span>
            <html:link styleClass="ts-admin-header__current" href="${contextPath}/WorkflowUdfViewAction.do?method=page&udfId=${udf.id}&id=${id}&workflowId=${flow.id}">
                <c:out value="${udf.caption}" escapeXml="true"/>
            </html:link>
        </c:if>
    </div>
</div>
