...
/**
 * Количество потраченных часов пользователями в за период (дни, недели, месяцы - в зависимости от длительности проекта) и сумма этих часов
 * @author winzard
 *
 */
@Immutable
public class PersonalActivityChart extends Chart {

	public PersonalActivityChart(SecuredTaskBean task, List<SecuredTaskBean> list,
			String options) {
	...
	public String calculate() throws GranException{
	    ...
	    try {
            ...
            String id = String.valueOf(System.currentTimeMillis());
             s.append("<div id=\"pactivitychart_div").append(id).append("\"></div>\n");
             s.append("<script type=\"text/javascript\">\n");
             s.append("google.load(\'visualization\', \'1.0\', {\'packages\':[\'corechart\']});\n");
             s.append("google.setOnLoadCallback(drawpactivitychart").append(task.getId()).append(");\n");
             s.append("function drawpactivitychart").append(task.getId()).append("() {\n");
             ...
             s.append("data.addColumn(\'number\', \'").append(u.getName()).append("\');\n");
      		...
      		s.append("		      var chart = new google.visualization.ComboChart(document.getElementById(\'pactivitychart_div").append(id).append("\'));\n");
